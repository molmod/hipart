...
# TODO: fix nomenclature of net charges
    ...
    def do_atgrids_overlap_matrix(self):
        ...
        def do_one_kind(kind):
            ...
            if kind!="alpha" and self.context.wavefn.alpha_orbitals is orbitals:
                ...
                for i in xrange(molecule.size):
                    setattr(self.atgrids[i], "%s_overlap_matrix" % kind, self.atgrids[i].alpha_overlap_matrix)
                return
            ...
            num_orbitals = self.context.wavefn.num_orbitals
            for i in xrange(molecule.size):
                matrix = self.atgrids[i].load("%s_%s_overlap_matrix" % (self.prefix, kind))
                if matrix is None:
                    some_failed = True
                else:
                    matrix = matrix.reshape((num_orbitals, num_orbitals))
                setattr(self.atgrids[i], "%s_overlap_matrix" % kind, matrix)
            ...
            if some_failed:
                ...
                for i, number_i in enumerate(molecule.numbers):
                    pb()
                    if getattr(self.atgrids[i], "%s_overlap_matrix" % kind) is None:
                        ...
                        for j1 in xrange(num_orbitals):
                            for j2 in xrange(j1+1):
                                ...
                                matrix[j2,j1] = value
                        setattr(self.atgrids[i], "%s_overlap_matrix" % kind, matrix)
                        self.atgrids[i].dump("%s_%s_overlap_matrix" % (self.prefix, kind), matrix)
                pb()

            filename = os.path.join(self.context.outdir, "%s_%s_overlap_matrices.txt" % (self.prefix, kind))
            f = file(filename, "w")
            print >> f, "number of orbitals:", num_orbitals
            print >> f, "number of atoms: ", molecule.size
            for i, number_i in enumerate(molecule.numbers):
                print >> f, "Atom %i: %s" % (i, periodic[number_i].symbol)
                matrix = getattr(self.atgrids[i], "%s_overlap_matrix" % kind)
                for row in matrix:
            ...
            for i in xrange(molecule.size):
                for j in xrange(i+1):
                    ...
                    if i==j:
                        # compute valence
                        bo = get_correlation(
                            self.atgrids[i].natural_overlap_matrix,
                            self.atgrids[j].natural_overlap_matrix,
                            n_natural, o_natural
                        )
                        ...
                        bo = 2*(
                            get_correlation(
                                self.atgrids[i].alpha_overlap_matrix,
                                self.atgrids[j].alpha_overlap_matrix,
                                n_alpha, o_alpha
                            )+
                            get_correlation(
                                self.atgrids[i].beta_overlap_matrix,
                                self.atgrids[j].beta_overlap_matrix,
                                n_beta, o_beta
                            )
                        )
